...
class Interpreter(object):
    ...
    def expr(self):
        ...
        # cumulative result across 1 or more operations
        result = self.current_token.value
        self.eat(INTEGER)

        # compute operation for each op token
        while self.current_token.type in (PLUS, MINUS, MULT, DIV):
            ...
            # integer to be calculated with current result
            right = self.current_token
            self.eat(INTEGER)

            # apply current operation and update result
            if op.type == PLUS:
